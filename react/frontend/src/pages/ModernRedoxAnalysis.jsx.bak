...
const ModernRedoxAnalysis = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const chunkSize = shouldChunk ? (maxFidelity ? 50000 : 25000) : null;
      ...
      if (selectedView === 'timeseries' || selectedView === 'rolling' || selectedView === 'details') {
        ...
        const useMaxFidelity = selectedView === 'details' ? true : maxFidelity;
        // Enforce explicit cadence when NOT max fidelity: 2H and fixed 6 depths
        const suggestedResolution = useMaxFidelity ? null : '2H';
        const maxDepths = useMaxFidelity ? 9999 : 6;
        const allowedDepths = useMaxFidelity ? undefined : [10, 30, 50, 100, 150, 200];
        const targetPoints = undefined;
        const sourceMode = useMaxFidelity ? 'raw' : 'processed';
