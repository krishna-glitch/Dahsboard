...
class RedshiftDataAPIConnection:
        ...
        for key, value in used_params.items():
            ...
            if value is None:
                ...
                for i, item in enumerate(value):
                    ...
                    parameters.append({
                        'name': param_name,
                        'value': str(item)  # All list items treated as strings for now
                    })
    ...
    def execute_statement(self, sql: str, parameters: Optional[List[Dict]] = None) -> str:
        ...
        try:
            ...
            # Add parameters if provided - keep AWS Redshift Data API expected structure
            if parameters:
                # parameters should be a list of {'name': '...', 'value': {'stringValue'|...}}
                            ...
                            if isinstance(value, str):
                                ...
                                if '-' in value and ':' in value and len(value) > 10:
                                    try:
                                        ...
                                        logger.debug(f"DateTime parsing failed for '{value}': {e}")
