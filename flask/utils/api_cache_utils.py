    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(f"💾 [SMART CACHE] Stored {endpoint_name} result for {ttl}s "
                           f"(fidelity: {requested_fidelity}, type: {data_type}, "
                           f"method: {compression_result.method.value}, "
                           f"ratio: {compression_result.compression_ratio:.1f}x)")
            except Exception as e:
                ...
                try:
                    ...
                    logger.info(f"💾 [CACHED] Stored {endpoint_name} result for {ttl}s "
                               f"(fidelity: {requested_fidelity}) - {compression_ratio:.1f}x compression")
                    ...
                    logger.info(f"💾 [CACHED] Stored {endpoint_name} result for {ttl}s "
                               f"(fidelity: {requested_fidelity}) - uncompressed")
